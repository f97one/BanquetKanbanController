...
public class BanquetSchedule implements Serializable {
     ...
     * 宴会ID
     */
    @DatabaseField(unique = true, columnName = FIELD_BANQUET_ID, foreign = true, foreignColumnName = "BanquetSummary.BANQUET_ID")
    private Integer banquetId;
    /**
     * 表示シーケンス番号
     */
    @DatabaseField(unique = true, columnName = FIELD_SEQ_NO, defaultValue = "1")
    private int seqNo;
    /**
     * 発動までの経過時間／時間
     */
